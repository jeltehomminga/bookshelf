...
import {useBooksSearch, useRefetchBookSearchQuery} from 'utils/books.exercise'

function DiscoverBooksScreen({user}) {
  const [query, setQuery] = React.useState('')
  const [queried, setQueried] = React.useState(false)
  const {books, error, isLoading, isError, isSuccess} = useBooksSearch({
    query,
    token: user.token,
  })

  useRefetchBookSearchQuery(user.token)
